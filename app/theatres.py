    ...
    if request.method == 'GET':
        ...
        if not data:
            return render_template(
                "AvailableTheatres.html", fields=[], rows=[], table_title=f"No theatre available on {data['input_date']} and time slot: {data['time_slot']}"
            )
        return render_template(
            "AvailableTheatres.html", fields=data[0], rows=data[1],
