...
def view_audiences_bought_tickets():
    ...
    if request.method == "GET":
        ...
        if data:
            return render_template(
                "AudiencesBoughtTickets.html",
                movies=movies,
                fields=data[0],
                rows=data[1],
                table_title="Audiences Bought Tickets",
            )
        else:
            return render_template(
                "AudiencesBoughtTickets.html",
                movies=movies,
                table_title="No Bought Tickets for This Movie",
            )
...
def delete_audience():
    if request.method == "POST":
        ...
        res = postgres_aws.get(
            f"""select * from Audience where username = '{username}'"""
        )  # check if it is an audience
        if len(res) == 0:
...
def list_movies():
    ...
    return render_template(
        "TableView.html", fields=data[0], rows=data[1], table_title="Movies"
    )
...
def buy_ticket():
    if request.method == "POST":
        ...
        if len(res) != 0:
            return render_template(
                "AudienceBuyTicket.html",
                error="You have already bought a ticket for this session!",
            )
        ...
        if ticket_count >= capacity:
            return render_template(
                "AudienceBuyTicket.html", error="There is no space in the theatre!"
            )
        ...
        if flag == False:
            return render_template(
                "AudienceBuyTicket.html",
                error="You have to watch all the predecessors of the movie before!",
            )
        else:
...
WHERE
    ...
    data = postgres_aws.get_rows_and_fields_from_sql(query)
    return render_template(
        "TableView.html", fields=data[0], rows=data[1], table_title="Tickets"
    )
