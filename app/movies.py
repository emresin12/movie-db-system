...
def create_movie():
    ...
    if request.method=='GET':
        return render_template(
            "MoviesCreate.html", genres=genres
        )
    elif request.method=='POST':
        input_request = {'movie_id': request.form['movie_id'], 'movie_name': request.form['movie_name'],
                         'duration': request.form['duration'], 'genre_list': request.form.getlist('genre_list')}
        try:
            ...
            flash(str(e))
            return render_template(
                "MoviesCreate.html", genres=genres, error=e
            )
...
class CreateMovieSessionRequest(BaseModel):
    ...
    def insert_to_database(self):
        # get max session_id
        max_session_id = postgres_aws.get("select max(session_id) as max_session_id from moviesession")[0]['max_session_id']
...
def results():
    if request.method=="POST":
        ...
        return render_template(
            "MoviesViewRatingsByAudience.html", fields=data[0], rows=data[1],
